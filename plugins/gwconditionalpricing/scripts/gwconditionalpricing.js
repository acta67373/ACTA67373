...
window.GWConditionalPricing = function( formId, pricingLogic, basePrices ) {
    ...
    this.init = function() {
        ...
        var gwcp             = this,
        	...
        	onClickSelector  = [ 'input[type="checkbox"]', 'input[type="radio"]' ].join(', '),
        	onKeyUpSelector  = [ 'input[type="text"]', 'input[type="hidden"]', 'textarea' ].join(', ' ),
            keyupTimeout;
    ...
    GWConditionalPricing.getFormElement = function( formId ) {
		...
		var formElem = $( gform.applyFilters( 'gpcp_form_element', '#gform_' + formId ) );
		
        if( formElem.length <= 0 )
        	formElem = $( '#gform_wrapper_' + formId );
    ...
    this.updatePricing = function( triggerFieldId ) {
        ...
        for( var productId in this._pricingLogic ) {
            ...
            var pricingLevels = this._pricingLogic[ productId ],
                isProduct     = parseInt( productId ) == triggerFieldId,
