jQuery(function($){
	...
	function getPaginationPages(e) {
		...
		var tab = $this.parents('#visibility-tabs').find('.themify-visibility-type-options[data-type=' + type + ']');
		var id = $this.parents('.themify-cm-conditions-container').data('item');
		var selected = $('.menu-location-menus[data-item=' + id + '] .locations-row-links input').val();
		$.ajax({
			url: ajaxurl,
			type: 'post',
			data: {
	...
	$('body').on('click', '#visibility-tabs .themify-visibility-tab', getPaginationPages);

	// Generate pagination for tab
	function showPaginationPage(e) {
		e.preventDefault();
		var $this = $(this);
		var tab = $this.parents('.themify-visibility-options');
		var items_inner = $this.parents('.themify-visibility-items-inner');
		var items_page = $('.themify-visibility-items-page', items_inner).addClass('is-hidden');
		var pagination = $('.themify-visibility-pagination', items_inner);
		var current_page = parseFloat($('.themify-visibility-pagination .current', items_inner).text());
		var go_to_page = 1;
	...
	function add_assignment( $menu_row, new_id, selected_menu, condition_value ) {
		...
		var conditions_num = $('.cm-replacement-button').length + 1;
	...
	$('body').on('click', '.themify-cm-conditions-container .uncheck-all', function(){
